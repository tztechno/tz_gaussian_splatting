...
const GaussianSplattingViewer = () => {
  ...
  const parsePLY = async (text) => {
    ...
    if (newGaussians.length > 0) {
      ...
      console.log('Normalized gaussians:', newGaussians.length);
      console.log('Bounds:', { minX, maxX, minY, maxY, minZ, maxZ });
      console.log('Center:', { centerX, centerY, centerZ });
      console.log('Scale:', scale);
      console.log('Sample gaussian:', newGaussians[0]);

      setGaussians(newGaussians);
      updateBuffers(newGaussians);
      alert(`✅ ${newGaussians.length}個のガウシアンを読み込みました`);
    } else {
      alert('❌ ガウシアンデータが見つかりませんでした');
    }
  ...
  const render = () => {
    ...
    if (gaussians.length === 0) {
      ...
      if (ctx) {
        ...
        ctx.fillText('PLYファイルを読み込んでください', 50, 50);
      }
      return;
    }
  ...
  const handleCameraUpload = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        console.log('Raw camera data:', data);
        
        let cameraArray = Array.isArray(data) ? data : data.cameras || [];
        console.log('Camera array length:', cameraArray.length);
        console.log('First camera:', cameraArray[0]);
        ...
        console.log('Processed cameras:', newCameras.length);
        console.log('First processed camera:', newCameras[0]);
      ...
      <header className="bg-gray-800 border-b border-gray-700 p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => {
                console.log('PLY button clicked');
                document.getElementById('plyFileInput').click();
              }}
